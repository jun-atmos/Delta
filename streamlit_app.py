...
with st.expander("홈페이지 설명서"):
    ...
    with tab3:
        ...
        st.text("체감온도, 불쾌지수는 기상청에서 사용하는 식들을 사용합니다.(기상청에서 사계절을 재조정함에 따라 그에 맞춰 설정했습니다.)")
        st.text("불쾌지수는 기온과 습도의 조합으로 사람이 느끼게 되는 온도를 표현합니다.")
        st.text("겨울철 체감온도는 기온과 바람으로 사람이 느끼는 추위를 나타내는 지수. (풍속이 1.3m/s이상이자 기온이 10도 이하일 때 산출됩니다. 이에 해당되지 않을 경우에는 풍속을 빼고, 기온은 더해주었습니니다.)")
        st.text("봄과 가을은 일반적으로 날씨가 덥거나 춥지 않기 때문에, 기온이 실제 체감온도와 크게 차이가 없을 수 있지만, 체감온도를 사용하면 바람과 습도를 고려할 수 있기 때문에 이를 사용합니다. 기온이 20도 이상인 경우에는 여름철 체감온도를 사용합니다. 20도 미만인 경우에는 겨울철 체감온도를 사용합니다.")
        st.text("데이터를 실시간으로 분자료를 들고 왔으며, 기온은 1분 평균 기온을, 풍속은 10분 평균 풍속을 사용합니다. (이는 기상청에서 지수 계산할 때 쓰는 풍속입니다.)")
        st.text("강수량은 15분 누적 강수량으로, 만약 NaN 이라면 0으로 채워주었고, 상대습도는 1분 평균 상대 습도를 사용합니다. 시정값은 1분 평균 시정(m)을 사용합니다. (이때, 관측기기의 샘플링은 1초간격(첨단화))")
        st.text("시정은 제주도 관측소 41곳에서 13곳밖에 없으므로, 특정 지점에서의 값을 주변 값들의 가중평균으로 계산(거리 가까울수록 더 큰 가중치)하는 Inverse Distance Weighting(역거리 가중치 보간(공간보간))을 사용합니다. 즉, 이미 알고 있는 값들로부터 알고자 하는 값을 추정한 것입니다.")
